...
public class UITextureInspector : UIWidgetInspector
{
	...
	override protected bool DrawProperties ()
	{
		...
		if (mTex.material == null || mTex.hasDynamicMaterial)
		{
            ...
            Texture texMast = EditorGUILayout.ObjectField("Texture Mask", mTex.TextureMask, typeof(Texture), false) as Texture;
            if (mTex.TextureMask != texMast)
            {
                NGUIEditorTools.RegisterUndo("Texture Mask Selection", mTex);
                mTex.TextureMask = texMast;
            }

            float cutOff = EditorGUILayout.Slider("CutOff", mTex.CutOff, 0f, 1f);

            if (cutOff != mTex.CutOff)
            {
                NGUIEditorTools.RegisterUndo("CutOff", mTex);
                mTex.CutOff = cutOff;
            }
