...
public class UIAtlas : MonoBehaviour
{
	...
	public class Sprite
	{
        ...
        // Cut the Border for some pixels to make a good blood sprite [Jerrylai_2014-11-26 11:31:39_start]
        public int cutBorderLeft    = 0;
        public int cutBorderRight   = 0;
        public int cutBorderTop     = 0;
        public int cutBorderBottom  = 0;
        // Cut the Border for some pixels to make a good blood sprite [Jerrylai_2014-11-26 11:31:39_end]
    ...
    Dictionary<string, int> dictStr2Idx = new Dictionary<string, int>();  // add by thomasmeng
	public Sprite GetSprite (string name)
	{
		...
		else if (!string.IsNullOrEmpty(name))
		{
            //add by thomas start
            int idx = 0;
            if (dictStr2Idx.TryGetValue(name, out idx))
            {
                if (idx < sprites.Count && !string.IsNullOrEmpty(sprites[idx].name) && name == sprites[idx].name)
                {
                    return sprites[idx];
                }
            }
			...
			for (int i = 0, imax = sprites.Count; i < imax; ++i)
			{
				...
				{
                    if ( dictStr2Idx.ContainsKey( name ) == false )
                    {
                        dictStr2Idx.Add( name , i );
                    }
					return s;
				}
			}
