...
public class UIRoot : MonoBehaviour
{
    ...
    static Camera Cam
    {
        get
        {
            if (cam == null)
            {
                cam = GameObject.FindWithTag("2DUICamera").GetComponent<Camera>();
            }
	...
	public int activeHeight
	{
		get
		{
            ...
            if (scalingStyle == Scaling.FixedSize) // add by konglingzhao
            {
                if ((float)Screen.width / Screen.height >= 1.5)
                {
                    return manualHeight;
                }
                else
                {
                    return (960 * Screen.height) / Screen.width ;
                }
            }

#if UNITY_IPHONE || UNITY_ANDROID ||UNITY_WEBPLAYER// add by tangqi
	...
	public float GetPixelSizeAdjustment (int height)
	{
        ...
        if ( scalingStyle == Scaling.FixedSize )
        {
            ...
            return (float)activeHeight / height;//改了屏幕分辨率自适应后，人物头顶名称出现偏差，所以用本行替代了上行[Added By : Jerrylai 2014-09-03 15:56:47]
        }
	...
	void Update ()
	{
		if (mTrans != null)
		{
            ...
            {
                calcActiveHeight = (int)(calcActiveHeight / Cam.orthographicSize);
            }
